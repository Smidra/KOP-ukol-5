...
def solve_brute_cut(self):
    self.complexity_brute = 0
    builder = []
    self.brute_cut(self.limit_weight, self.min_cost, 0, 0, builder)
    return


# Brute force (with weight cutting) recursion function
def brute_cut(self, lim_weight, min_cost, total_cost, size, solution_builder):
    ...
    if (lim_weight >= 0):
        ...
        tmp.append(1)
        self.brute_cut(lim_weight=lim_weight - self.things_array[size].weight,
                       min_cost=min_cost,
    ...
    tmp = copy.copy(solution_builder)
    tmp.append(0)
    self.brute_cut(lim_weight=lim_weight,
                   min_cost=min_cost,
